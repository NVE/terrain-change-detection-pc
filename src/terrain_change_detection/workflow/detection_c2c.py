...
def _run_streaming_c2c(cfg, data, alignment, *, visualizer, show_plots):
    ...
    if cfg.alignment.reference == "t2":
        ...
        if files_tgt:
            ...
            logger.info(
                "Using pre-transformed files for T1 target: %d files", len(files_tgt)
            )
            transform_src = None
        else:
            logger.info(
        ...
        files_src = (
            data.pc2_data.get("aligned_file_paths") or data.pc2_data["file_paths"]
        )
    ...
    if use_parallel:
        ...
        if getattr(cfg.detection.c2c, "mode", "euclidean") != "euclidean":
            logger.warning(
                "C2C mode '%s' not supported in streaming; using euclidean.",
    ...
    try:
        if (
            ...
            and cfg.visualization.backend == "plotly"
        ):
...
def _run_inmemory_c2c(cfg, data, alignment, *, visualizer, show_plots):
    ...
    if export_c2c_pc or export_c2c_raster:
        _export_c2c(
            ...
            src,
            c2c_res,
            export_pc=export_c2c_pc,
            export_raster=export_c2c_raster,
        )
