...
class DataDiscovery:
    ...
    def __init__(
        ...
        *,
        ...
        source_type: str = "hoydedata",
        loader: Optional[PointCloudLoader] = None,
    ):
        """
        ...
        self.metadata_dir_name = (
            metadata_dir_name if source_type == "hoydedata" else None
        )
    ...
    def scan_areas(self, user_area_name: Optional[str] = None) -> Dict[str, AreaInfo]:
        ...
        if len(subdirs) > 0:
            ...
            time_period_like = sum(
                1 for d in subdirs if d.name.replace("-", "").isdigit()
            )
            if time_period_like == len(subdirs) and len(subdirs) > 0:
                logger.warning("=" * 80)
                logger.warning("CONFIGURATION WARNING:")
                logger.warning(
                    f"All subdirectories in {self.base_dir} look like time periods."
                )
                logger.warning(f"Found: {[d.name for d in subdirs]}")
                logger.warning("")
                logger.warning(
                    "It appears you may have set base_dir to an area folder instead of its parent."
                )
                logger.warning(
                    f"Expected structure: base_dir/area/time_period/{'data/' if self.source_type == 'hoydedata' else ''}*.laz"
                )
                logger.warning(
                    f"Current structure appears to be: base_dir/time_period/{'data/' if self.source_type == 'hoydedata' else ''}*.laz"
                )
                logger.warning("")
                logger.warning(
                    "If this is intentional, the subdirectories will be treated as 'area' names."
                )
                logger.warning("=" * 80)
        ...
        for area_path in subdirs:
            ...
            if datasets:
                ...
                logger.info(
                    f"Found {len(datasets)} time periods for area: {area_name}: {list(datasets.keys())}"
                )
        ...
        for time_path in area_path.iterdir():
            if time_path.is_dir() and not time_path.name.startswith("."):
                ...
                if laz_files:
                    ...
                    try:
                        (
                            dataset_info.total_points,
                            dataset_info.bounds,
                            dataset_info.per_file_stats,
                        ) = self._get_dataset_stats(laz_files)
                    except Exception as e:
                        logger.error(
                            f"Error getting stats for dataset {dataset_info.area_name}/{dataset_info.time_period}: {e}"
                        )
...
class BatchLoader:
    ...
    def __init__(
        self, loader: Optional[PointCloudLoader] = None, streaming_mode: bool = False
    ):
        """
    ...
    def load_dataset(
        ...
        *,
            ...
            try:
                ...
                if max_points_per_file and len(points) > max_points_per_file:
                    indices = np.random.choice(
                        len(points), max_points_per_file, replace=False
                    )
        ...
        if not all_points:
            logger.warning(
                f"No valid point cloud data found in dataset {dataset_info.area_name}/{dataset_info.time_period}"
            )
            return {
                "points": np.array([]),
                "attributes": {},
        ...
        logger.info(
            f"Combined dataset: {len(combined_points)} points from {len(all_points)} files"
        )
    ...
    def _prepare_streaming_dataset(self, dataset_info: DatasetInfo) -> Dict:
        ...
        """
        logger.info(
            f"Preparing streaming dataset for {dataset_info.area_name}/{dataset_info.time_period}"
        )
        ...
        return {
            ...
            "metadata": {
                ...
                "ground_percentage": (100.0 * total_ground / total_all)
                if total_all > 0
                else None,
    ...
    def _load_separate_files(
        self, dataset_info: DatasetInfo, max_points_per_file: Optional[int] = None
    ) -> Dict:
        """
        ...
        for laz_file in dataset_info.laz_files:
            try:
                ...
                if max_points_per_file and len(pc_data["points"]) > max_points_per_file:
                    indices = np.random.choice(
                        len(pc_data["points"]), max_points_per_file, replace=False
                    )
