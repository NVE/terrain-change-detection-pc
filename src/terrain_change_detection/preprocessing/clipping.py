...
class AreaClipper:
    ...
    def __init__(self, geometry: Union["Polygon", "MultiPolygon"]):
        ...
        if not SHAPELY_AVAILABLE:
            raise ImportError(
                "shapely is required for area clipping. Install it with: uv add shapely"
            )
        ...
        if not geometry.is_valid:
            logger.warning(
                "Input geometry is invalid, attempting to fix with buffer(0)"
            )
    ...
    @classmethod
    def from_file(
        cls, file_path: Union[str, Path], feature_name: Optional[str] = None
    ) -> "AreaClipper":
        """
    ...
    @classmethod
    def _from_geojson(
        cls, file_path: Path, feature_name: Optional[str] = None
    ) -> "AreaClipper":
        ...
        return cls._from_geojson_dict(
            geojson_data, str(file_path), feature_name=feature_name
        )
    ...
    def _from_geojson_dict(
        ...
        if feature_name:
            logger.info(
                f"Using feature '{feature_name}' with {len(polygons)} polygon(s)"
            )
        else:
            logger.info(
                f"Loaded {len(polygons)} polygon(s), combined into clipping boundary"
            )
    ...
    def clip(
        self, points: np.ndarray, *, return_mask: bool = False
        ...
        if n_in_bbox == 0:
            logger.debug(f"Clipping: 0/{n_points} points - all outside bounding box")
        ...
        logger.info(f"Clipped to {n_inside:,}/{n_points:,} points ({percentage:.1f}%)")
    ...
    def clip_with_attributes(
        self, points: np.ndarray, attributes: dict[str, np.ndarray]
    ...
    def get_statistics(self, points: np.ndarray) -> dict:
        ...
        return {
            ...
            "percentage_inside": float(100.0 * n_inside / n_total)
            if n_total > 0
            else 0.0,
            "percentage_outside": float(100.0 * n_outside / n_total)
            if n_total > 0
            else 0.0,
    ...
    def save_geojson(self, file_path: Union[str, Path]) -> None:
        ...
        geojson = {"type": "FeatureCollection", "features": [self.to_geojson()]}
    ...
    def transform_to_local(
        self, local_transform: "LocalCoordinateTransform"
    ) -> "AreaClipper":
        """
...
def clip_point_cloud_files(
    ...
    *,
    ...
    for input_file in input_files:
        ...
        if n_output == 0:
            logger.warning(
                f"No points remain after clipping {input_file.name}, skipping output"
            )
            continue
