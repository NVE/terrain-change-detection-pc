...
def apply_transform_to_files(
    ...
    *,
    ...
    logger.info(
        f"Ground only: {ground_only}, Classification filter: {classification_filter}"
    )
                ...
                for chunk in reader.chunk_iterator(chunk_points):
                    ...
                    if hasattr(chunk, "classification"):
                        ...
                        mask = create_classification_mask(
                            classes, ground_only, classification_filter
                        )
                    else:
