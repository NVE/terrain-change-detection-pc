...
def detect_gpu() -> GPUInfo:
    ...
    try:
        ...
        logger.info(f"{msg}: {e}")
        return GPUInfo(
            available=False, device_count=0, error_message="CuPy not installed"
        )

    except Exception as e:
        msg = f"GPU detection failed - GPU acceleration disabled: {e}"
        logger.warning(msg)
        return GPUInfo(available=False, device_count=0, error_message=str(e))
...
def get_optimal_batch_size(
    point_count: int, bytes_per_point: int = 32, max_memory_fraction: float = 0.8
) -> int:
    """
