...
class GPUNearestNeighbors:
    ...
    def fit(self, X: np.ndarray) -> "GPUNearestNeighbors":
        ...
        if self.backend_ == "sklearn-cpu":
            ...
            try:
                ...
                _max_samples = int(
                    _os.environ.get("TCD_STORE_CPU_COPY_MAX_SAMPLES", "300000")
                )
            except Exception:
                _max_samples = 300000
            try:
            ...
            if _max_samples > 0 and n_samples <= _max_samples:
                ...
                self._train_X_cpu = np.ascontiguousarray(
    ...
    def radius_neighbors(
        ...
        if self.backend_ == "sklearn-cpu":
            return self._model.radius_neighbors(
            ...
            logger.info(
                "radius_neighbors using CPU fallback with sklearn on stored training copy"
            )
            cpu_model = SklearnNN(
                n_neighbors=self.n_neighbors,
