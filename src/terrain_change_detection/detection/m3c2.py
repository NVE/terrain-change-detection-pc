...
class M3C2Detector:
    ...
    def autotune_m3c2_params(
        ...
        logger.info(
            ...
            params.max_depth,
            params.normal_scale
            if params.normal_scale is not None
            else params.projection_scale,
        )
    ...
    def autotune_m3c2_params_from_headers(
        ...
        try:
            ...
            raise ValueError(
                "laspy required for autotune_m3c2_params_from_headers"
            ) from e
    ...
    def compute_m3c2_original(
        ...
        *,
        ...
        with (
            capture_c_streams_to_logger(
                logger, level=logging.DEBUG, include_patterns=["Building KDTree"]
            ),
            redirect_stdout_stderr_to_logger(
                logger, level=logging.DEBUG, pattern="Building KDTree"
            ),
        ):
        ...
        if uncertainties is not None:
            try:
                ...
                if (
                    hasattr(uncertainties, "dtype")
                    and uncertainties.dtype.names is not None
                ):
    ...
    def compute_m3c2_streaming_files_tiled(
        ...
        *,
        ...
        default_halo = max(
            float(params.cylinder_radius), float(params.projection_scale), normal_r
        )
        ...
        def _tile_bounds(i: int, j: int) -> tuple[Bounds2D, Bounds2D]:
            ...
            inner = Bounds2D(
                ox, oy, min(ox + tile_size, gb.max_x), min(oy + tile_size, gb.max_y)
            )
            outer = Bounds2D(
        ...
        for rec in uniq_tiles:
            ...
            chunks_t1 = list(
                reader1.stream_points(bbox=outer_global, transform=local_transform)
            )
            if not chunks_t1:
            ...
            chunks_t2 = list(
                reader2.stream_points(bbox=outer_global, transform=local_transform)
            )
            if not chunks_t2:
    ...
    def compute_m3c2_streaming_files_tiled_parallel(
        ...
        *,
        ...
        logger.info(
            f"Processing {len(tiles_with_data)} tiles with core points (out of {len(all_tiles)} total)"
        )
        ...
        for tile in tiles_with_data:
            ij_key = (tile.i, tile.j)
            files_t1_tile = [
                str(f) for f, b in t1_bounds if bounds_intersect(tile.outer, b)
            ]
            files_t2_tile = [
                str(f) for f, b in t2_bounds if bounds_intersect(tile.outer, b)
            ]
    ...
    def compute_m3c2_plane_based(
        ...
        raise NotImplementedError(
            "M3C2 (plane-based) will be implemented in the next phase."
        )
