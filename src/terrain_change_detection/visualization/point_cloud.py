...
class PointCloudVisualizer:
    ...
    def visualize_clouds(
        ...
        if len(point_clouds) != len(names):
            raise ValueError(
                "The number of point clouds must match the number of names."
            )
        if sample_size:
    ...
    def visualize_distance_histogram(
        self, distances: np.ndarray, title: str, bins: int = 256
    ):
        if self.backend != "plotly":
    ...
    def visualize_c2c_points(
        self,
        source_points: np.ndarray,
        distances: np.ndarray,
        sample_size: Optional[int] = None,
        title: str = "C2C distances",
    ):
        """
        ...
        """
        return self.visualize_m3c2_corepoints(
            source_points, distances, sample_size=sample_size, title=title
        )
    ...
    def _get_plotter(self):
        if self.backend == "pyvistaqt":
            if BackgroundPlotter is None:
                raise ImportError(
                    "pyvistaqt is not installed. Install with 'uv add pyvistaqt PySide6'."
                )
