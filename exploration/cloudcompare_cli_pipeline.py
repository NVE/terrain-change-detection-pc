...
class PipelineConfig:
    ...
    subsample_param: float = (
        0.1  # distance for SPATIAL, count for RANDOM, level for OCTREE
    )
...
def build_command_file(config: PipelineConfig) -> List[str]:
    ...
    if config.m3c2_params_file and config.m3c2_params_file.exists():
        ...
        commands.append(
            "# Generate one via CloudCompare GUI: Tools -> Distances -> M3C2 -> Save params"
        )
        commands.append(
            "# Or run with --create-m3c2-params to create a default template"
        )
...
def execute_cloudcompare(
    cc_bin: str, command_file: Path, capture_output: bool = True
...
def analyze_output_file(output_path: Path) -> Dict[str, Any]:
    ...
    if suffix in (".las", ".laz"):
        try:
            ...
            distance_fields = [
                d
                for d in las.point_format.dimension_names
            ...
            if distance_fields:
                ...
                if len(valid) > 0:
                    ...
                    threshold = (
                        results["distance_stats"]["mean"]
                        + 2 * results["distance_stats"]["std"]
                    )
                    significant = np.sum(valid > threshold)
                    results["distance_stats"]["significant_changes"] = int(significant)
                    results["distance_stats"]["significant_percent"] = float(
                        significant / len(valid) * 100
                    )
            ...
            if "distance_stats" in results:
                ...
                print(
                    f"    Significant changes (>2σ): {stats['significant_changes']:,} ({stats['significant_percent']:.1f}%)"
                )

        except ImportError:
            print(
                "  [WARNING] laspy not installed. Install with: pip install laspy[lazrs]"
            )
        ...
        try:
            ...
            print(
                "  [WARNING] plyfile not installed. Install with: pip install plyfile"
            )
...
Examples:
  ...
  python cloudcompare_cli_pipeline.py --enable-sor --analyze
    ...
    io_group.add_argument(
        "--out", type=Path, default=default_out, help="Output file path"
    )
    io_group.add_argument(
        "--cc-bin", type=str, default=None, help="Path to CloudCompare executable"
    )

    # ICP parameters
    icp_group = parser.add_argument_group("ICP Registration")
    icp_group.add_argument(
        "--icp-iter", type=int, default=60, help="Maximum ICP iterations (default: 60)"
    )
    icp_group.add_argument(
        "--icp-overlap",
        type=int,
    ...
    pre_group.add_argument(
        "--enable-sor", action="store_true", help="Enable Statistical Outlier Removal"
    )
    pre_group.add_argument(
        "--sor-neighbors",
        type=int,
    ...
    pre_group.add_argument(
        "--enable-subsampling", action="store_true", help="Enable spatial subsampling"
    )
    pre_group.add_argument(
        "--subsample-dist",
        type=float,
    ...
    exec_group.add_argument(
        "--keep-cmd", action="store_true", help="Keep command file for debugging"
    )
    exec_group.add_argument(
        "--analyze", action="store_true", help="Analyze output file after processing"
    )
