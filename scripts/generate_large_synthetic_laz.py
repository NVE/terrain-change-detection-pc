...
def make_terrain_tile(
    ...
    Z += 1.0 * np.sin(0.03 * X) * np.cos(0.025 * Y) + 0.8 * np.cos(0.04 * (X + Y))
...
def sample_points_from_surface(
    ...
    points = np.column_stack([X[yi, xi], Y[yi, xi], Z[yi, xi]])
...
def write_laz(
    ...
    las.intensity = (
        (100 + 50 * rng.standard_normal(points.shape[0])).clip(0, 255).astype(np.uint16)
    )
...
def main():
    ...
    print(
        f"  Expected points per tile: ~{int(tile_grid_nx * tile_grid_ny * sample_density):,}"
    )
    print(
        f"  Expected total points per epoch: ~{int(tiles_x * tiles_y * tile_grid_nx * tile_grid_ny * sample_density):,}"
    )
    ...
    for i, (cx, cy, r, mag, ctype) in enumerate(terrain_changes, 1):
        print(
            f"  {i}. {ctype:10s} at ({cx:6.0f}, {cy:6.0f}), r={r:.0f}m, Δz={mag:.1f}m"
        )
    ...
    for i in range(tiles_x):
        for j in range(tiles_y):
            ...
            print(
                f"\n  Tile {tile_idx + 1:2d}/{tiles_x * tiles_y} (i={i}, j={j}) at ({x_offset:.0f}, {y_offset:.0f})...",
                end=" ",
            )
            ...
            pts_t1 = sample_points_from_surface(
                X, Y, Z, density=sample_density, seed=seed_base + 100
            )
            ...
            pts_t2 = sample_points_from_surface(
                X, Y, Z2, density=sample_density, seed=seed_base + 200
            )
            ...
            if tile_idx == 0:
                print(
                    f"T1={len(pts_t1):,} pts (Z: {pts_t1[:, 2].min():.2f} to {pts_t1[:, 2].max():.2f}), "
