...
def test_default_config_outofcore_disabled():
    """Test that default config has out-of-core disabled."""
    cfg = load_config(None)  # Load default
    
    assert cfg.outofcore.enabled == False
    assert cfg.outofcore.streaming_mode == True  # But ready to use when enabled
...
def test_large_scale_profile_outofcore_enabled():
    """Test that large scale profile has out-of-core enabled."""
    cfg = load_config("config/profiles/large_scale.yaml")
    
    assert cfg.outofcore.enabled == True
    assert cfg.outofcore.streaming_mode == True
    assert cfg.outofcore.save_transformed_files == True
    assert cfg.outofcore.tile_size_m == 1000.0
    assert cfg.outofcore.halo_m == 50.0
    assert cfg.outofcore.chunk_points == 2_000_000
    assert cfg.outofcore.output_dir == "data/processed"
...
def test_synthetic_profile_outofcore_settings():
    ...
    assert cfg.outofcore.enabled == False
    assert cfg.outofcore.streaming_mode == True
    assert cfg.outofcore.save_transformed_files == False  # Don't save for small datasets
