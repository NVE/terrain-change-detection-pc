...
class TestDataDiscovery(unittest.TestCase):
    ...
    def test_scan_areas(self):
        ...
        for area_name, area_info in areas.items():
            ...
            for time_period, dataset_info in area_info.datasets.items():
                ...
                if dataset_info.bounds:
                    ...
                    print(
                        f"      Bounds: X({bounds['min_x']:.1f}-{bounds['max_x']:.1f}), Y({bounds['min_y']:.1f}-{bounds['max_y']:.1f}), Z({bounds['min_z']:.1f}-{bounds['max_z']:.1f})"
                    )
        ...
        for time_period in ["2007", "2013"]:
            ...
            self.assertGreater(
                len(dataset.laz_files), 0, f"Should have LAZ files for {time_period}"
            )
            self.assertIsInstance(dataset.total_points, int)
            self.assertGreater(
                dataset.total_points, 0, f"Should have points for {time_period}"
            )
...
class TestBatchLoader(unittest.TestCase):
    ...
    def test_load_separate_files(self):
        ...
        print(
            f"Testing separate file loading for: {dataset_info.area_name}/{dataset_info.time_period}"
        )
        ...
        for i, pc_data in enumerate(file_data):
            ...
            if len(pc_data["points"]) > 0:
                ...
                print(
                    f"    Bounds: X({points[:, 0].min():.1f}-{points[:, 0].max():.1f}), "
    ...
    def test_load_separate_files_with_max_points(self):
        ...
        result = self.batch_loader._load_separate_files(
            dataset_info, max_points_per_file=max_points
        )
